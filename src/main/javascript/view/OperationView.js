...
SwaggerUi.Views.OperationView = Backbone.View.extend({
  ...
  formatXml: function (xml) {
    ...
    fn = function (ln) {
      ...
      padding = ((function () {
        var m, ref1, results
        results = []
        for (j = m = 0, ref1 = indent; ref1 >= 0 ? m < ref1 : m > ref1; j = ref1 >= 0 ? ++m : --m) {
          results.push('  ')
        }
        return results
      })()).join('')
  ...
  showStatus: function (response) {
    ...
    var form = $('.sandbox', $(this.el)),
      ...
      authz = this.model.clientAuthorizations.authz
    if (authz) {
      for (var key in authz) {
        if (authz[key].type === 'header') {
          requestHeaders[authz[key].name] = authz[key].value
        }
      }
    }
